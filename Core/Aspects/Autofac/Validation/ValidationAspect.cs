...
{
    public class ValidationAspect : MethodInterception // sen bir metotInterceptionsın. içindeki metotları ezebilirsin. Bizim ASPECT'immiz bu
    {
        ...
        // benim canım bunu ezmek istiyor. (swh) => bu doğrulama olduğundan metodun başında yapılır. o yüzden bunu ezdik.
        protected override void OnBefore(IInvocation invocation) // metotd interceptiondaki onbefore metodu bu
        {
            var validator = (IValidator)Activator.CreateInstance(_validatorType);  //bu reflection: çalışma anında birşeyleri çalıştırabilmenizi sağlıyor. ör: newleme işini çalışma anında yaptırma. Product validator ın instance ını oluştur.IValidator tipinde kullanılabilir hale getir.
